...
def train (data,Attr,conceptVals, concept):
    ...
    for cVal in conceptVals:
        countConcept[cVal],conceptProbs[cVal] = proAttr(data,concept,cVal)
    AttrConcept = {}
    probability_list = {}
    for att in Attr:
        ...
        for val in Attr[att]:
            AttrConcept[att][val] = {}
            a, probability_list[att][val] = proAttr(data,att,val)
            for cVal in conceptVals:
...
import pandas as pd
df = pd.read_csv("/home/sahyadri/cod/tennis.csv")
...
examples = pd.read_csv("/home/sahyadri/cod/tennis.csv")
